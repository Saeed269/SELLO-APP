        ...
        {Array.from({ length: totalSellos }).map((_, i) => {
          ...
          return (
            <div key={i} style={{ aspectRatio: '1', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: marcado ? (esUltimo ? '#FFD700' : '#fff') : 'rgba(255,255,255,0.2)', border: marcado ? 'none' : '1.5px solid rgba(255,255,255,0.35)' }}>
              {marcado && <IconSVG path={esUltimo ? premioPath : selloPath} size={14} color={col} />}
            </div>
          )
        })}
      </div>
            ...
            {Array.from({ length: totalSellos }).map((_, i) => {
              ...
              return (
                <div key={i} style={{ aspectRatio: '1', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: marcado ? (esUltimo ? '#FFD700' : 'rgba(255,255,255,0.85)') : 'rgba(255,255,255,0.12)', border: marcado ? 'none' : '1.5px solid rgba(255,255,255,0.25)' }}>
                  {marcado && <IconSVG path={esUltimo ? premioPath : selloPath} size={14} color="#1C1C1E" />}
                </div>
              )
            })}
          </div>
          <p style={{ margin: '8px 0 0', fontSize: '12px', color: 'rgba(255,255,255,0.85)', fontWeight: '500' }}>🎁 Premio: {ultimoPremio}</p>
        </div>
      </div>
...
export default function Tarjeta() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'tarjetas', filter: `id=eq.${tarjeta.id}` },
  ...
  const premioPath = ICONOS_PREMIO[diseno?.premioIcon] || ICONOS_PREMIO.gift

  const tarjetaProps = { diseno, nombre: negocio?.nombre, totalSellos, sellosActuales, premios, qrCliente, premioGanado, selloPath, premioPath }
...
const s = {
  root:     { minHeight: '100dvh', backgroundColor: '#f5f5f5', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '1rem', boxSizing: 'border-box' },
  salirBtn: { position: 'fixed', top: '1rem', right: '1rem', background: 'none', border: '1px solid #ddd', borderRadius: '20px', padding: '4px 14px', fontSize: '0.8rem', color: '#888', cursor: 'pointer', zIndex: 100 },
  wrapper:  { width: '100%', maxWidth: '420px', marginTop: '3.5rem', boxSizing: 'border-box' },
}