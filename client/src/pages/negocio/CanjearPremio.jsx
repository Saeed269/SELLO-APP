      ...
      <div style={s.content}>
        {canjeado ? (
          <div style={s.successCard}>
            ...
            <p style={{ color: COLORS.primary, fontWeight: 'bold', fontSize: '1.1rem', marginBottom: '1.5rem' }}>{negocio?.premio}</p>
            <button onClick={() => navigate('/negocio/escanear')} style={s.btnPrimary}>Escanear otro cliente</button>
            <button onClick={() => navigate('/negocio/dashboard')} style={s.btnSecondary}>Volver al inicio</button>
          </div>
        ) : (
          <div style={s.card}>
            <p style={s.icon}>🏆</p>
            ...
            <div style={s.premioBox}>
              ...
              <p style={{ color: COLORS.primary, fontWeight: 'bold', fontSize: '1.2rem', margin: 0 }}>{negocio?.premio}</p>
            </div>
            ...
            {error && <p style={{ color: COLORS.danger, fontSize: '0.9rem', marginBottom: '1rem' }}>{error}</p>}
            <button onClick={handleCanjear} style={s.btnPrimary} disabled={loading}>
              {loading ? 'Canjeando...' : '✓ Confirmar canje'}
            </button>
            <button onClick={() => navigate('/negocio/dashboard')} style={s.btnSecondary}>Cancelar</button>
          </div>
        )}
      </div>
    </div>
...
const s = {
  container:   { minHeight: '100dvh', backgroundColor: '#f9fafb', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  content:     { maxWidth: '440px', width: '100%', padding: '1rem', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  ...
  premioBox:   { backgroundColor: '#FDF5EE', borderRadius: '12px', padding: '1rem 2rem', marginBottom: '1.5rem', border: `2px solid ${COLORS.primary}44`, width: '100%', boxSizing: 'border-box' },
  btnPrimary:  { padding: '0.85rem', backgroundColor: COLORS.primary, color: '#fff', border: 'none', borderRadius: '10px', fontSize: '1rem', fontWeight: 'bold', cursor: 'pointer', width: '100%', marginBottom: '0.75rem' },
  btnSecondary:{ padding: '0.75rem', backgroundColor: 'transparent', color: '#888', border: '1.5px solid #e0e0e0', borderRadius: '10px', fontSize: '0.95rem', cursor: 'pointer', width: '100%' },
}