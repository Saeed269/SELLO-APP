...
function ResultadoCliente({ tarjeta, maxSellos, onAñadirSello, onNuevo, onVolver, loading }) {
  ...
  return (
    <div style={s.resultCard}>
      <h3 style={s.clienteNombre}>{tarjeta.clientes?.nombre || 'Cliente'}</h3>
...
export default function EscanerQR() {
  ...
  const añadirSello = async () => {
    ...
    if (error) { setError('Error al añadir sello: ' + error.message) }
    else { setResultado({ ...resultado, sellos_actuales: nuevosSellos }) }
    setLoading(false)
  }
