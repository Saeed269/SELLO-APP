...
const FAQS = [
  ...
  { pregunta: '¿Cómo añado un sello a un cliente?',        respuesta: 'Hay dos formas: escanea el QR del cliente desde el Dashboard pulsando "Escanear QR del Cliente", o ve a la sección Clientes, busca al cliente y pulsa "+ Sello".' },
  { pregunta: '¿Cómo canjeo el premio de un cliente?',     respuesta: 'Cuando un cliente completa su tarjeta, escanea su QR y serás redirigido automáticamente a la pantalla de canje.' },
  { pregunta: '¿Puedo cambiar el número de sellos o el premio?', respuesta: 'Sí, ve a Mi Tarjeta → Configuración y modifica los sellos, el premio o la caducidad.' },
  { pregunta: '¿Puedo personalizar el diseño de la tarjeta?',    respuesta: 'Sí, en Mi Tarjeta → Diseño puedes cambiar el estilo, el color, el efecto decorativo y los iconos.' },
    ...
    <div style={{ borderBottom: '1px solid #f0f0f0' }}>
      ...
      >
        ...
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
          style={{ flexShrink: 0, transform: abierto ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.2s' }}>
          <polyline points="6 9 12 15 18 9"/>
        </svg>
      </button>
      {abierto && <p style={{ margin: '0 0 1rem', fontSize: '0.85rem', color: '#555', lineHeight: 1.6, textAlign: 'left' }}>{respuesta}</p>}
    </div>
  )
}
      ...
      <div style={s.modal} onClick={e => e.stopPropagation()}>
        {enviado ? (
          ...
          <>
            ...
            <p style={{ margin: '0 0 1.25rem', fontSize: '0.85rem', color: '#888' }}>Cuéntanos tu duda o problema y te ayudaremos lo antes posible.</p>
            <textarea placeholder="Escribe tu mensaje aquí..." value={mensaje} onChange={e => setMensaje(e.target.value)} rows={5} style={s.textarea} />
            <div style={{ display: 'flex', gap: '10px', marginTop: '1rem' }}>
              <button onClick={onClose} style={s.btnCancelar}>Cancelar</button>
              <button onClick={handleEnviar} disabled={!mensaje.trim() || enviando} style={{ ...s.btnEnviar, opacity: !mensaje.trim() ? 0.5 : 1 }}>
      ...
      <main style={s.main}>
        <div style={s.inner}>
          ...
          <div style={s.section}>
            ...
            {FAQS.map((faq, i) => <FaqItem key={i} pregunta={faq.pregunta} respuesta={faq.respuesta} />)}
          </div>
...
const s = {
  ...
  btnEnviar:   { flex: 1, padding: '0.85rem', backgroundColor: COLORS.primary, color: '#fff', border: 'none', borderRadius: '10px', fontSize: '0.92rem', fontWeight: '700', cursor: 'pointer' },
}